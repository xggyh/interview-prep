...
COMPANY_ORDER = ["Apple HM 面", "Google FDE Summary", "Googlyness 标准答", "Google FDE", "Google L5", "FDE", "OpenAI", "Google"]

# Per-company brand color (used for the company badge tint)
COMPANY_COLOR = {
    "Apple HM 面": "#000000",  # Apple black
    "Googlyness 标准答": "#f9ab00",  # Google yellow — behavioral answers
...
def render_index(questions, type_groups, company_groups, recency_sorted):
    ...
    types_meta = [
        ...
        ("Apple HM 面", "🍎 Apple HM 面（30 道 hiring-manager 首轮高频题 · 极详细英文背诵稿 + 中文要点 + 边界 + 追问）"),
        ("Googlyness 标准答", "🟡 Googlyness 标准答（26 道 Google behavioral 真题 · EN 背诵稿 + 中文速记 · 基于真实简历）"),
