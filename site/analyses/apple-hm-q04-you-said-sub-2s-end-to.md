...
> 🇨🇳 是 observability 层里记录的 per-hop 时间戳——每一级都吐出 enter/exit 的时间戳，绑在一个 turn ID 上，所以对每一通真实的电话，我都能把毫秒数归因到 ASR、LLM 还是 TTS，而不是一个合成的测试。我看的是分布，主要是 p50 和 p95。在报确切数字之前我会先去拉一下线上的实时数据——我现在给你的是个"形状"。

**Q: You optimized TTFT with KV-cache reuse — what exactly is cached and what's the win?**
**中**: 你用 KV-cache 复用优化了 TTFT——到底缓存的是什么，收益又是什么?
...
> 🇨🇳 prompt 里那段跨 turn 稳定的前缀——system instruction 和 tool schema——它的 KV 只算一次然后复用，所以每一轮的 prefill 只处理新的用户 turn 加上检索进来的 context。这个收益随着你 prompt 里有多少是静态的而放大；对我们来说，system+tools 这段前缀是一大块固定内容，所以它很可观。在 Apple 那边，这跟他们公开的那个 KV-cache sharing 是同一个思路，能砍掉 prefill 的内存——在他们报告里大约 37.5%——只不过是放在 on-device 上做。

**Q: Could you have used a smaller/faster model to cut TTFT instead?**
**中**: 你本可以换一个更小/更快的模型来砍 TTFT，为什么不这么做?
...
> 🇨🇳 可以，而且那是真实的 trade-off。更小的模型会降低 TTFT，但可能会掉 task accuracy 和合规的遵守度，这在债务催收里是不可接受的。我的做法是：让模型保持足够强、足够安全，然后通过 serving 的优化和 overlap 去把 latency 买回来，而不是靠缩小这个"大脑"。如果我被迫要用更小的模型，我会用在特定任务上更紧的 post-training 来补偿——而这恰恰就是 on-device adapter 那套故事。

**Q: What's your p95, not your average? The tail is what hurts.**
**中**: 你的 p95 是多少，不是均值? 真正伤人的是 tail。
...
> 🇨🇳 同意——这是个对的问题。均值是舒舒服服在 budget 之下的；tail 才是偶尔的慢 tool call、或者一次很长的生成把它顶上去的地方。我明确地 monitor p95，而 tail 最大的贡献者是 tool latency 和异常长的 completion。我会给你线上的 p95 而不是去猜——但我之所以精确地 track 它，正是因为，像你说的，主叫感受到的是 tail，不是均值。

**Q: Where would on-device change this budget?**
**中**: 换成 on-device，这个 budget 会在哪里变?
...
> 🇨🇳 TTFT 的性质会变——没有到 GPU 的网络 round trip 了，但你是在一部手机的算力和内存预算上做 prefill，所以 quantization 和 KV-cache 的那些决策就变成了全部的关键。对 on-device 的 turn 来说，network 开销基本上消失了；PCC 的 turn 会重新引入一个 round trip，但带着隐私保证。做 budget 的这套功夫是完全一样的——动的只是那些 line item。
